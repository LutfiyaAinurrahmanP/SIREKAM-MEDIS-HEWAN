...
describe("GET /client/appointments/:id", () => {
  beforeEach(async () => {
    await createData("appointments");
  });
  afterEach(async () => {
...
describe("PATCH /client/appointments/:id", () => {
  beforeEach(async () => {
    await createData("appointments");
  });
  afterEach(async () => {
  ...
  it("should update an existing appointment", async () => {
    ...
    const response = await supertest(web)
      .patch(`/client/appointments/${appointment?.id}`)
      .set("SESSION-TOKEN", "token-client")
      .send({
        pet_id: pet?.id,
        created_by: user?.id,
  ...
  it("should return error if appointment not found", async () => {
    ...
    const response = await supertest(web)
      .patch(`/client/appointments/99999`)
      .set("SESSION-TOKEN", "token-client")
      .send({
        pet_id: pet?.id,
        created_by: user?.id,
...
describe("DELETE /client/appointments/:id", () => {
  ...
  it("should delete an existing appointment", async () => {
    const appointment = await AppointmentsTest.getAppointmentsId();
    const response = await supertest(web)
      .delete(`/client/appointments/${appointment?.id}`)
      .set("SESSION-TOKEN", "token-client");
  ...
  it("should return error if appointment not found", async () => {
    const response = await supertest(web)
      .delete(`/client/appointments/99999`)
      .set("SESSION-TOKEN", "token-client");
